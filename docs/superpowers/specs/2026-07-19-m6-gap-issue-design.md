...
外部写入默认关闭（`sjherp.github.issue.enabled=false`），且即使开启仍要求候选显式审核通过。仅 ADMIN/BOSS 可查看候选、审核、触发、重试和治理；普通登录用户、Agent 不具备 Issue 外发能力。Token、仓库和 API 地址只从配置/密钥注入，绝不硬编码。
...
候选幂等键为 `tenant_id + cluster_key`，数据库唯一约束保证并发聚类不重复；投递采用 `PENDING/APPROVED/SENDING/SENT/FAILED`，数据库行锁抢占，外部成功后回写编号和 URL。失败保留错误类型/次数并允许 ADMIN/BOSS 重试；未知异常不得把已成功外部 Issue 重发。全程通过应用/领域服务写入并由 `@Audited` 记录，未提供删除接口。
