...
GitHub 出站响应契约为 `IssueResponse(number, url, labels)`；`labels` 是外部验收字段，必须包含固定的 `sjherp-gap`、业务模块和严重度标签才允许本地候选进入 SENT。

## 范围与候选规则

仅 `gap_record.status=NEW` 的记录进入候选；已 triage、开发中、解决或驳回的记录不重复外发。候选按业务模块、严重度、标题/缺失能力/期望行为的完整 Unicode NFKC 规范化元组（小写、折叠空白）计算 SHA-256 确定性键；展示标题等字段可单独限长，但哈希输入绝不截断。不使用 LLM 或模糊相似度。每个候选保留最早记录为代表、最多 20 条场景样本和全部来源 gap 编号，内容只读、禁止物理删除。
