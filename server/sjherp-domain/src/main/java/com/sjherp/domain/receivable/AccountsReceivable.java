...
public final class AccountsReceivable implements AuditTarget {
     ...
     *
     * <p>语义（设计真源 §2 核销反向引擎，与 {@link #settle} 对称）：
     * <ul>
     *   <li>{@code amount <= 0} → {@link IllegalArgumentException}；先 {@code setScale(AMOUNT_SCALE, ROUNDING)} 归一；</li>
     ...
     *   <li>{@link ReceivableStatus#REVERSED} 子账不可反向（其 settled 恒 0），抛 {@link IllegalStateException}。</li>
     * </ul>
     *
     * <p>只动 {@link #settledAmount} / {@link #status}（其余 final 字段不变）：原始 {@link #amount} 永不变
    ...
    public void unsettle(BigDecimal amount) {
        if (status == ReceivableStatus.REVERSED) {
            throw new IllegalStateException("应收[" + sourceDocNo + "] 已冲销，不可反向核销");
        }
        if (amount == null || amount.signum() <= 0) {
        ...
        if (newSettled.signum() < 0) {
            throw new IllegalArgumentException("反向核销额 " + delta.toPlainString()
                    + " 超过已核销额 " + settledAmount.toPlainString() + "（应收[" + sourceDocNo + "]）");
        }
        this.settledAmount = newSettled;
        if (newSettled.signum() == 0) {
