...
/**
 * 供应商档案聚合根（模式样板：商品档案 {@code domain/catalog/Product}）。
 *
 * <p>档案不是单据：没有状态机流转，只有启用/停用两态（{@link ArchiveStatus}）。
 * 档案**不可物理删除**（历史单据引用必须永远可追溯），下线即停用。
 *
 * <p>code 租户内唯一（唯一性经仓储校验，由 {@link SupplierService} 把关；
 * 数据库 tenant_id+code 联合唯一键兜底）。
 */
public final class Supplier implements AuditTarget {
    ...
    /** 供应商编码（租户内唯一；可手填或由编号规则 SUP-年月-序号 自动生成） */
    private String code;
    ...
    /** 新建供应商，初始状态为启用（id 由仓储落库后回填） */
    public Supplier(String code, String name, String contactPerson, String contactPhone,
                    String address, String taxNo, SettlementMethod settlementMethod, String operator) {
    ...
    private Supplier(Long id, String code, String name, String contactPerson, String contactPhone,
                     String address, String taxNo, SettlementMethod settlementMethod, ArchiveStatus status,
    ...
    public static Supplier restore(long id, String code, String name, String contactPerson,
                                   String contactPhone, String address, String taxNo,
                                   SettlementMethod settlementMethod, ArchiveStatus status,
                                   String createdBy, Instant createdAt, String updatedBy, Instant updatedAt) {
        return new Supplier(id, code, name, contactPerson, contactPhone, address, taxNo,
                settlementMethod, status, createdBy, createdAt, updatedBy, updatedAt);
    }

    /**
     * 整体更新基础信息（code 唯一性由 {@link SupplierService} 经仓储校验）。
     * 停用供应商也允许修正信息，但状态只能走 enable/disable。
     */
    ...
    public void enable(String operator) {
        if (status == ArchiveStatus.ENABLED) {
            throw new IllegalArgumentException("供应商[" + code + "] 已是启用状态，无需重复启用");
        }
        this.status = ArchiveStatus.ENABLED;
        touch(operator);
    }
    ...
    public void disable(String operator) {
        if (status == ArchiveStatus.DISABLED) {
            throw new IllegalArgumentException("供应商[" + code + "] 已是停用状态，无需重复停用");
        }
    ...
    public void assignId(long id) {
        if (this.id != null) {
            throw new IllegalStateException("供应商 id 已分配，不可重复分配: " + this.id);
        }
        this.id = id;
    }
    ...
    private static String validateCode(String code) {
        ...
        if (trimmed.length() > CODE_MAX_LENGTH) {
            throw new IllegalArgumentException("供应商编码不能超过 " + CODE_MAX_LENGTH + " 个字符");
        }
        return trimmed;
    }
    ...
    private static String validateName(String name) {
        ...
        if (trimmed.length() > NAME_MAX_LENGTH) {
            throw new IllegalArgumentException("供应商名称不能超过 " + NAME_MAX_LENGTH + " 个字符");
        }
        return trimmed;
    }
    ...
    public String auditSummary() {
        return "编码=" + AuditTarget.text(code) + ", 名称=" + AuditTarget.text(name)
                ...
                + ", 结算方式=" + settlementMethod.label() + ", 状态=" + status.label();
    }
}
