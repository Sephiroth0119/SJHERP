...
public class CategoryService {
    ...
    @Audited(action = "category.create", targetType = "category")
    ...
    @Audited(action = "category.rename", targetType = "category")
     ...
     * 返回被删除类目快照（M2-T07：供审计切面记录目标与摘要；operator 为审计操作人）。
     */
    @Audited(action = "category.delete", targetType = "category")
    public Category delete(long id, String operator) {
        Category category = get(id);
