...
public final class PurchaseReceiptLine {
    ...
    /**
     * 回滚本次开票数量（M4-T07b 采购发票红冲时由 {@link PurchaseReceiptService} 同事务回写）。
    ...
    void subtractInvoiced(BigDecimal delta) {
        ...
        if (next.signum() < 0) {
            throw new IllegalArgumentException("行号 " + lineNo + " 回滚开票数量 "
                    + delta.toPlainString() + " 超过已开票量 " + invoicedQty.toPlainString());
        }
        this.invoicedQty = next.setScale(CostingStrategy.UNIT_COST_SCALE, CostingStrategy.ROUNDING);
    }
