...
class GapIssueScheduledPublisherTest {
    @Test void 人工审核开启时保留PENDING但投递APPROVED和FAILED() {
        ...
        when(service.dispatchable(3,200)).thenReturn(List.of(approved,failed));
        new GapIssueScheduledPublisher(service,true,3).publish();
        verify(service).deliver(approved.id(),"system:gap-issue"); verify(service).deliver(failed.id(),"system:gap-issue");
    }
    @Test void 达到最大重试次数不再投递() {
        GapIssueService service=mock(GapIssueService.class); when(service.cluster(anyString())).thenReturn(List.of());
        GapIssueCandidate failed=candidate(1,GapIssueStatus.FAILED,3); when(service.list()).thenReturn(List.of(failed));
        when(service.dispatchable(3,200)).thenReturn(List.of(failed));
        new GapIssueScheduledPublisher(service,false,3).publish(); verify(service,never()).deliver(failed.id(),"system:gap-issue");
    }
