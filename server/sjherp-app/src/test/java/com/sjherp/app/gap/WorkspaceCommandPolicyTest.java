...
class WorkspaceCommandPolicyTest {
 @Test void rejectsWorkspaceEscapeAndInvalidBranch(){WorkspacePolicy p=new WorkspacePolicy(Path.of("C:/repo"));assertThatThrownBy(()->p.validate("codex/dev/x",Path.of("C:/repo/../secret"))).isInstanceOf(IllegalArgumentException.class);assertThatThrownBy(()->p.validate("main",Path.of("C:/repo/x"))).isInstanceOf(IllegalArgumentException.class);}
