...
/**
 * partner API 统一错误响应（仅作用于本包的控制器，不影响其他包既有处理器）。
 *
 * <p>错误体与既有契约一致：{"error": "..."}（同 CatalogExceptionHandler 风格）。
 */
@RestControllerAdvice(basePackageClasses = PartnerExceptionHandler.class)
public class PartnerExceptionHandler {

    /** 档案不存在 → 404 {"error": "..."} */
    @ExceptionHandler(PartnerNotFoundException.class)
    public ResponseEntity<Map<String, String>> handleNotFound(PartnerNotFoundException e) {
        return error(HttpStatus.NOT_FOUND, e.getMessage());
    }

    /** 业务规则拒绝（编码重复、重复启停、结算方式非法等）→ 400 {"error": "..."} */
