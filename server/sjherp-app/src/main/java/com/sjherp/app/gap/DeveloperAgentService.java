...
public class DeveloperAgentService {
    ...
    public DeveloperAgentTask run(long id,String operator){if(!runner.available())throw new GapIssueDisabledException("developer agent execution is disabled");DeveloperAgentTask queued=get(id);GapIssueCandidate candidate=candidates.findById(queued.candidateId()).orElseThrow(()->new GapIssueNotFoundException(queued.candidateId()));String lease=tasks.claim(id,Instant.now()).orElseThrow(()->new GapIssueStateException("task is already leased or not retryable"));DeveloperAgentTask claimed=get(id);try {DeveloperAgentRunner.Result result=runner.run(new DeveloperAgentRunner.RunRequest(claimed,candidate));if(result.generatedArtifacts().isEmpty()||result.generatedArtifacts().stream().anyMatch(String::isBlank)||!result.targetedTestsGreen()){tasks.markFailed(id,DeveloperAgentTaskStatus.RUNNING,lease,"QUALITY_GATE","missing artifacts or targeted tests");return get(id);}tasks.transition(id,DeveloperAgentTaskStatus.RUNNING,DeveloperAgentTaskStatus.TESTING,lease,result.generatedArtifacts(),true,false,false,null);if(!result.fullTestsGreen()||!result.ciGreen()||result.ciEvidence()==null||result.ciEvidence().isBlank()){tasks.markFailed(id,DeveloperAgentTaskStatus.TESTING,lease,"QUALITY_GATE","full tests or CI evidence missing");return get(id);}tasks.transition(id,DeveloperAgentTaskStatus.TESTING,DeveloperAgentTaskStatus.AWAITING_REVIEW,lease,result.generatedArtifacts(),true,true,true,result.ciEvidence());return get(id);} catch(RuntimeException ex){try{tasks.markFailed(id,get(id).status(),lease,ex.getClass().getSimpleName(),truncate(ex.getMessage(),500));}catch(RuntimeException suppressed){ex.addSuppressed(suppressed);}throw ex;}}
