...
/**
 * 下达工单（DRAFT → APPROVED，M5-T07，HIGH 确认）。
 */
public class ReleaseWorkOrderTool implements Tool {

    private static final Logger log = LoggerFactory.getLogger(ReleaseWorkOrderTool.class);

    private final TransactionalWorkOrderService workOrderService;

    public ReleaseWorkOrderTool(TransactionalWorkOrderService workOrderService) {
        this.workOrderService = Objects.requireNonNull(workOrderService, "workOrderService");
    }

    @Override
    public String name() { return "release_work_order"; }

    @Override
    public String description() {
        return "下达工单：将草稿工单（DRAFT）推进到已审核（APPROVED）状态，允许后续领料/投产。"
                + "必填：doc_no（工单单号）。";
    }
    ...
    public ToolResult execute(Map<String, Object> arguments, ToolContext context) {
        try {
            ...
            WorkOrder wo = workOrderService.release(docNo, operator);
            log.info("工单下达成功：docNo={}, operator={}", docNo, operator);
            return ToolResult.ok(CreateWorkOrderTool.toData(wo));
        } catch (IllegalArgumentException e) {
            return ToolResult.fail(e.getMessage());
        } catch (Exception e) {
            return ToolResult.fail("工单下达失败：" + e.getMessage());
        }
    }
}
