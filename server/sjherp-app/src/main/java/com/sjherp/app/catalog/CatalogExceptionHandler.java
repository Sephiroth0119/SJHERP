...
/**
 * catalog API 统一错误响应（仅作用于本包的控制器，不影响 chat 包既有处理器）。
 *
 * <p>错误体与既有契约一致：{"error": "..."}（同 ChatSessionController 的 404/400 风格）。
 */
...
public class CatalogExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleNotFound(CatalogNotFoundException e) {
        return error(HttpStatus.NOT_FOUND, e.getMessage());
    }

    /** 业务规则拒绝（编码重复、层级超限、重复启停、引用保护等）→ 400 {"error": "..."} */
