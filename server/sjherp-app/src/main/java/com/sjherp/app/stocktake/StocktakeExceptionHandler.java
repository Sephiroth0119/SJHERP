...
/**
 * 盘点单 API 统一错误响应（仅作用于本包的控制器，不影响其他包既有处理器）。
 * 错误体与既有契约一致：{"error": "..."}（同 InventoryExceptionHandler 风格）。
 *
 * <p>领域异常映射（M3-T03 验收契约）：
 * <ul>
 *   <li>{@link StockCountNotFoundException} / 仓库/商品不存在 → 404；</li>
 *   <li>{@link IllegalStateTransitionException} → 409（如重复审核、对已完成单过账）；</li>
 *   <li>{@link IllegalStateException} → 409（如非草稿状态修改实盘）；</li>
 *   <li>{@link InsufficientStockException} → 400（盘亏出库超量，默认禁负库存）；</li>
 *   <li>{@link IdempotencyConflictException} → 409（过账幂等键冲突）；</li>
 *   <li>业务规则拒绝（停用、数量非法、零库存盘盈缺成本等）→ 400。</li>
 * </ul>
 */
@RestControllerAdvice(basePackageClasses = StocktakeExceptionHandler.class)
public class StocktakeExceptionHandler {

    /** 盘点单不存在 → 404 */
    @ExceptionHandler(StockCountNotFoundException.class)
    public ResponseEntity<Map<String, String>> handleStockCountNotFound(StockCountNotFoundException e) {
    ...
    /** 业务规则拒绝（停用、数量非法、零库存盘盈缺成本、状态过滤值非法等）→ 400 */
