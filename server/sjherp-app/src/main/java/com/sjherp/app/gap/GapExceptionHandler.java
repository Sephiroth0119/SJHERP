...
/**
 * gap API 统一错误响应（仅作用于本包的控制器，不影响 chat/catalog 既有处理器）。
 *
 * <p>错误体与既有契约一致：{"error": "..."}。
 */
...
public class GapExceptionHandler {
    ...
    /** 缺口不存在 → 404 {"error": "..."} */
    @ExceptionHandler(GapRecordNotFoundException.class)
    public ResponseEntity<Map<String, String>> handleNotFound(GapRecordNotFoundException e) {
        return error(HttpStatus.NOT_FOUND, e.getMessage());
    }

    /** 业务规则拒绝（非法状态流转、非法枚举参数等）→ 400 {"error": "..."} */
