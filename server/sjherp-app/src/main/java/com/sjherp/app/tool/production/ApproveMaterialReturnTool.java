...
/**
 * 审核退料单（DRAFT → APPROVED，M5-T07，HIGH 确认）。
 */
public class ApproveMaterialReturnTool implements Tool {

    private static final Logger log = LoggerFactory.getLogger(ApproveMaterialReturnTool.class);

    private final MaterialReturnAppService materialReturnAppService;

    public ApproveMaterialReturnTool(MaterialReturnAppService materialReturnAppService) {
        this.materialReturnAppService = Objects.requireNonNull(materialReturnAppService, "materialReturnAppService");
    }

    @Override
    public String name() { return "approve_material_return"; }

    @Override
    public String description() {
        return "审核退料单（DRAFT → APPROVED）。审核通过后可过账执行实际库存入库（按原领料成本）。"
    ...
    public ToolResult execute(Map<String, Object> arguments, ToolContext context) {
        try {
            ...
            MaterialReturn ret = materialReturnAppService.approve(docNo, operator);
            log.info("退料单审核成功：docNo={}, operator={}", docNo, operator);
            return ToolResult.ok(CreateMaterialReturnTool.toData(ret));
        } catch (IllegalArgumentException e) {
            return ToolResult.fail(e.getMessage());
        } catch (Exception e) {
            return ToolResult.fail("退料单审核失败：" + e.getMessage());
        }
    }
}
