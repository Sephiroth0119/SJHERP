...
/**
 * 查询退料单（M5-T07，NORMAL，production:material）。
 *
 * <p>若提供 doc_no 则按单号精确查询；否则按 material_issue_doc_no / status 分页搜索。
 */
public class QueryMaterialReturnTool implements Tool {

    private final MaterialReturnAppService materialReturnAppService;

    public QueryMaterialReturnTool(MaterialReturnAppService materialReturnAppService) {
        this.materialReturnAppService = Objects.requireNonNull(materialReturnAppService, "materialReturnAppService");
    }

    @Override
    public String name() { return "query_material_return"; }

    @Override
    public String description() {
        return "查询退料单。提供 doc_no 精确查询单笔；否则按 material_issue_doc_no / status 分页搜索（page/size，默认第 1 页 10 条）。";
    }
        ...
        return """
                {
                  "type": "object",
                  "properties": {
                    "doc_no":                { "type": "string",  "description": "退料单号（精确查询，MR- 前缀）" },
                    "material_issue_doc_no": { "type": "string",  "description": "原领料单号过滤（可选）" },
                    "status":                { "type": "string",  "description": "状态过滤：DRAFT/APPROVED/COMPLETED/REVERSED" },
    ...
    public ToolResult execute(Map<String, Object> arguments, ToolContext context) {
        try {
            ...
            if (docNo != null) {
                MaterialReturn ret = materialReturnAppService.get(docNo);
            ...
            PageResult<MaterialReturn> result = materialReturnAppService.search(new MaterialReturnQuery(materialIssueDocNo, status, page, size));
            List<Map<String, Object>> items = result.items().stream()
                    .map(CreateMaterialReturnTool::toData)
                    .toList();
            ...
            return ToolResult.fail("查询退料单失败：" + e.getMessage());
        }
    }
