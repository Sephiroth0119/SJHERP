...
/**
 * 查询月末成本结转单（M5-T07，NORMAL，production:cost）。
 *
 * <p>若提供 doc_no 则按单号精确查询；否则按 period / status 分页搜索。
 */
public class QueryCostSettlementTool implements Tool {

    private final ProductionCostSettlementAppService settlementAppService;

    public QueryCostSettlementTool(ProductionCostSettlementAppService settlementAppService) {
        this.settlementAppService = Objects.requireNonNull(settlementAppService, "settlementAppService");
    }

    @Override
    public String name() { return "query_cost_settlement"; }

    @Override
    public String description() {
        return "查询月末成本结转单。提供 doc_no 精确查询单笔；否则按 period（账期 yyyyMM）/ status 分页搜索"
        ...
        return """
                {
                  "type": "object",
                  "properties": {
                    "doc_no": { "type": "string",  "description": "成本结转单号（精确查询，PC- 前缀）" },
                    "period": { "type": "string",  "description": "账期 yyyyMM 过滤（可选，如 202606）" },
    ...
    public ToolResult execute(Map<String, Object> arguments, ToolContext context) {
        try {
            ...
            if (docNo != null) {
                ProductionCostSettlement settlement = settlementAppService.get(docNo);
            ...
            PageResult<ProductionCostSettlement> result = settlementAppService.search(
                    new ProductionCostSettlementQuery(period, status, page, size));
            List<Map<String, Object>> items = result.items().stream()
                    .map(CreateCostSettlementTool::toData)
                    .toList();
            ...
            return ToolResult.fail("查询成本结转单失败：" + e.getMessage());
        }
    }
