...
public class JdbcDeveloperAgentTaskRepository implements DeveloperAgentTaskRepository {
    ...
    @Override public DeveloperAgentTask createIfAbsent(DeveloperAgentTask task, String operator) {
        ...
        jdbc.update("INSERT INTO developer_agent_task(tenant_id,candidate_id,idempotency_key,status,branch_name,workspace_path,runner_kind,attempt_count,ci_green,human_approved,generated_artifacts,targeted_tests_green,full_tests_green,ci_evidence,created_by,created_at,updated_at) VALUES(0,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)",
                task.candidateId(), task.idempotencyKey(), task.status().name(), task.branchName(), task.workspacePath(), task.runnerKind(), task.attemptCount(), task.ciGreen(), task.humanApproved(), jsonArray(task.generatedArtifacts()), task.targetedTestsGreen(), task.fullTestsGreen(), task.ciEvidence(), operator, now, now);
    ...
    @Override public Optional<String> claim(long id, Instant now) {
        ...
        int changed=jdbc.update("UPDATE developer_agent_task SET status='RUNNING',lease_token=?,attempt_count=attempt_count+1,updated_at=? WHERE tenant_id=0 AND id=? AND status IN ('QUEUED','FAILED') AND attempt_count<3",token,t,id);
        return changed==1?Optional.of(token):Optional.empty();
    }
    @Override public void transition(long id, DeveloperAgentTaskStatus expected, DeveloperAgentTaskStatus target, String leaseToken, java.util.List<String> artifacts, boolean targeted, boolean full, boolean ciGreen, String ciEvidence) {
        int changed=jdbc.update("UPDATE developer_agent_task SET status=?,generated_artifacts=?,targeted_tests_green=?,full_tests_green=?,ci_green=?,ci_evidence=?,lease_token=?,updated_at=? WHERE tenant_id=0 AND id=? AND status=? AND lease_token=?",target.name(),jsonArray(artifacts),targeted,full,ciGreen,ciEvidence,target==DeveloperAgentTaskStatus.TESTING?leaseToken:null,LocalDateTime.now(ZoneOffset.UTC),id,expected.name(),leaseToken);
        if(changed!=1) throw new IllegalStateException("developer task lease conflict");
    }
    @Override public void markFailed(long id, DeveloperAgentTaskStatus expected, String leaseToken, String failureType, String failureSummary){int c=jdbc.update("UPDATE developer_agent_task SET status='FAILED',failure_type=?,failure_summary=?,lease_token=NULL,updated_at=? WHERE tenant_id=0 AND id=? AND status IN ('RUNNING','TESTING') AND status=? AND lease_token=?",failureType,failureSummary,LocalDateTime.now(ZoneOffset.UTC),id,expected.name(),leaseToken);if(c!=1)throw new IllegalStateException("developer task failure lease conflict");}
    @Override public void approve(long id,String operator){int c=jdbc.update("UPDATE developer_agent_task SET status='APPROVED',human_approved=TRUE,updated_at=? WHERE tenant_id=0 AND id=? AND status='AWAITING_REVIEW' AND targeted_tests_green=TRUE AND full_tests_green=TRUE AND ci_green=TRUE AND ci_evidence IS NOT NULL AND ci_evidence<>''",LocalDateTime.now(ZoneOffset.UTC),id);if(c!=1)throw new IllegalStateException("task is not approvable");}
