...
public final class RestGitHubIssueClient implements GitHubIssueClient {
    ...
    public IssueResponse create(IssueRequest r){
        try { String body=json.writeValueAsString(java.util.Map.of("title",r.title(),"body",r.body(),"labels",r.labels()));
            ...
            if(!n.hasNonNull("number") || !n.hasNonNull("html_url")) throw new IllegalStateException("GitHub 创建响应缺少字段");
            return new IssueResponse(n.get("number").asLong(),n.get("html_url").asText());
    ...
    public Optional<IssueResponse> findByTraceMarker(String marker){
        try { var query="repo:"+repo+"+in:body+type:issue+"+java.net.URLEncoder.encode(marker,java.nio.charset.StandardCharsets.UTF_8);
            var response=http.send(request("/search/issues?q="+query).GET().build(),HttpResponse.BodyHandlers.ofString());
