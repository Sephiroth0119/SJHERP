...
面向**小型企业**的下一代 ERP 客户端，业务涵盖**进销存 + 财务**。
...
4. **数据一致性持续校验**。独立的**检查 Agent** 定时分析最新数据是否符合业务逻辑（库存账实、应收应付与单据勾稽、总账与明细账平衡等），发现错误立刻上报，不静默修复。
...
项目刚初始化，无任何代码。下一步：搭建 server 端 Maven 多模块骨架 + Agent 框架核心抽象（会话、工具注册、选项返回协议）。
